...
- **LayerStack (LS) 系 preset** (`edition-layerstacks*` および `edition-universal`) は
  `.bin` ヘッダーから bucket mode を判定する。`progress8kpabs` 系は progress バケット重み
  (`setoption name LS_PROGRESS_COEFF value <progress.bin path>`) が必要だが、
  `kingrank9` 系は `progress.bin` 不要。サーバー向けの build / 起動 / smoke test 手順は
  [`docs/server-layerstacks-build.md`](./server-layerstacks-build.md) を参照。
...
- [`docs/server-layerstacks-build.md`](./server-layerstacks-build.md) (サーバー向け LayerStacks NNUE build / 起動手順)
