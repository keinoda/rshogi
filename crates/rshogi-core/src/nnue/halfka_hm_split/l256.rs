...
mod tests {
    ...
    fn test_supported_specs() {
        assert_eq!(HalfKaHmSplitL256::SUPPORTED_SPECS.len(), 3);

        let spec = &HalfKaHmSplitL256::SUPPORTED_SPECS[0];
    ...
    fn test_l1_size() {
        ...
        for spec in HalfKaHmSplitL256::SUPPORTED_SPECS {
            assert_eq!(spec.l1, 256);
        }
    }
    ...
    fn test_supported_activations() {
        let activations: Vec<_> =
            HalfKaHmSplitL256::SUPPORTED_SPECS.iter().map(|s| s.activation).collect();
