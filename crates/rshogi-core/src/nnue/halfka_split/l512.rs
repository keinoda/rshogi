...
mod tests {
    ...
    fn test_supported_specs() {
        assert_eq!(HalfKaSplitL512::SUPPORTED_SPECS.len(), 9);

        // 8-64 CReLU
        let spec = &HalfKaSplitL512::SUPPORTED_SPECS[0];
    ...
    #[test]
    fn test_l1_size() {
        for spec in HalfKaSplitL512::SUPPORTED_SPECS {
            assert_eq!(spec.l1, 512);
        }
    }
    ...
    fn test_supported_activations() {
        let activations: Vec<_> =
            HalfKaSplitL512::SUPPORTED_SPECS.iter().map(|s| s.activation).collect();
    ...
    fn test_multiple_l2_l3_combinations() {
        let combinations: Vec<_> =
            HalfKaSplitL512::SUPPORTED_SPECS.iter().map(|s| (s.l2, s.l3)).collect();
