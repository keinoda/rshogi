...
mod tests {
    ...
    fn test_supported_specs() {
        assert_eq!(HalfKaMergedL1024::SUPPORTED_SPECS.len(), 9);

        // 8-64 CReLU
        let spec = &HalfKaMergedL1024::SUPPORTED_SPECS[0];
    ...
    #[test]
    fn test_l1_size() {
        for spec in HalfKaMergedL1024::SUPPORTED_SPECS {
            assert_eq!(spec.l1, 1024);
        }
    }
    ...
    fn test_supported_activations() {
        let activations: Vec<_> =
            HalfKaMergedL1024::SUPPORTED_SPECS.iter().map(|s| s.activation).collect();
    ...
    fn test_multiple_l2_l3_combinations() {
        let combinations: Vec<_> =
            HalfKaMergedL1024::SUPPORTED_SPECS.iter().map(|s| (s.l2, s.l3)).collect();
