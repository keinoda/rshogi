...
処理中は進捗率 %・残り時間 (ETA)・完了予定の絶対時刻を表示する。全モード
...
```
全体 38%  3.84M/10.0M  (shard 2/4)  8.3k/s  残り 12:40  完了 14:43
...
```
[00:01:23] ███████████░░░░░░░░░  52%  523.5k/1.00M  8.3k pos/s  残り 00:57  完了 14:32
```
...
```
[rescore] 全体 38.4% 3.84M/10.0M shard 2/4 (shard_003 52.3%) 8.3k pos/s elapsed 00:01:23 ETA 12:40 (完了 14:43)
```

- 全体（overall）の分母は glob 対象ファイルの件数を起動時に合算して算出する
  （`metadata().len() / 40` のみで全件 load しない）。`--limit` は各ファイルに適用され、
  `.done` 済み / レジューム分も全体進捗に反映される。
- 完了予定時刻は現在のローカル時刻 + ETA。速度が確定するまで（推論ウォームアップ中）は
  `完了 --:--` と表示される。
- ONNX モードで入力に破損 / パース不能レコードがあると、そのスキップ分は進捗に計上されず
  進捗表示が 100% に届かないことがある（rescore 出力は有効レコードのみで正常）。終了時に
